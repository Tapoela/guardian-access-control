...
class AnprReceiver extends Controller
{
    ...
    private function checkPlate(
        ...
        if ($blackEntry) {
            ...
            $white = $db->table('whitelist w')
                ...
                ->join('member_vehicles mv', 'mv.id = w.vehicle_id')
    ...
    private function triggerCameraAlarm(
        ...
        try {
            // Use the working endpoint and XML
            ...
            log_message('info', "[ANPR] Camera relay triggered on {$ip} ch{$channel} — HTTP {$code} — Response: {$res}");
